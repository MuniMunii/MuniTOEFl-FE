...
export default function ReadingCard(){
    ...
    const voucherReadingIsActive = vouchers?.data?.some(
  (v) => v.typeV === "reading"
) || false;
useEffect(()=>console.log(vouchers,voucherReadingIsActive),[vouchers])
  ...
  },
  {
    ...
    unlocked: voucherReadingIsActive,
  },
  {
    title: "Vocabulary",
    length: 20,
...
return dummyList.map((item) => (
    <Card className="w-full h-full min-h-[200px] relative overflow-hidden" key={'reading'+item.title}>
        <CardHeader className="flex justify-between"><h2 className="text-2xl font-semibold">Reading {item.title}</h2><p>0/{item.length}</p></CardHeader>
        <CardContent>
