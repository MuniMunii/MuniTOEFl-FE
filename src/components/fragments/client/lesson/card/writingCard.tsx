...
useEffect(()=>console.log(vouchers,voucherWritingIsActive),[vouchers])
  ...
  },
  {
    ...
    unlocked: voucherWritingIsActive,
  },
  {
    title: "Vocabulary",
    length: 20,
...
return dummyList.map((item) => (
    <Card className="w-full h-full min-h-[200px] relative overflow-hidden" key={'writing'+item.title}>
        <CardHeader className="flex justify-between"><h2 className="text-2xl font-semibold">{item.title}</h2><p>0/{item.length}</p></CardHeader>
        <CardContent>
