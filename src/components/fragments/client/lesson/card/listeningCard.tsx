...
export default function ListeningCard(){
    ...
    const voucherListeningIsActive = vouchers?.data?.some(
  (v) => v.typeV === "listening"
) || false;
useEffect(()=>console.log(vouchers,voucherListeningIsActive),[vouchers])
  ...
  },
  {
    ...
    unlocked: voucherListeningIsActive,
  },
  {
    title: "Vocabulary",
    length: 20,
...
return dummyList.map((item) => (
    <Card className="w-full h-full min-h-[200px] relative overflow-hidden" key={'listening'+item.title}>
        <CardHeader className="flex justify-between"><h2 className="text-2xl font-semibold">Listening {item.title}</h2><p>0/{item.length}</p></CardHeader>
        <CardContent>
