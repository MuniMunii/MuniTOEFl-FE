...
export default function GetCourseByType() {
  ...
  const { data, error, isLoading } = useQuery<metaTestDataType[]>({
    queryFn: async () => {
      const res = await apiClient.get(`/api/admin/test/tests?type=${type}`);
      return res.data.data ?? [];
    },
    queryKey: ["switch-typeTest-admin", type],
  });
  const mutateDeleteCourse = useMutation<any,Error,{id:string}>({
    mutationFn:async ({id})=>{
        const res=await apiClient.delete(`/api/admin/test/tests/${id}`)
        return res.data
    },
      onSuccess: (data) => {
        toast(data.message);
        clientQuery.invalidateQueries({ queryKey: ["switch-typeTest-admin"] });
      },
      onError: (err) => {
        toast(err.message);
      },
  ...
  function handleDelete(id: string) {
    return mutateDeleteCourse.mutate({ id });
  }
  function RenderCourseContent() {
    if (error) return;
    if (isLoading)
      return [1, 2, 3].map((val) => (
        <div className="h-[100px] w-full p-4 flex justify-between border border-gray-500/60 rounded-sm">
        ...
        <div className="border border-gray-400 rounded-md p-4 flex justify-between">
          <div className="w-fit">
            ...
            <div className="flex h-5 items-center space-x-4 text-sm mt-2">
              <p className="capitalize">
                {val.isFree ? "Free course" : "Locked Content"}
              </p>
              <Separator orientation="vertical" />
              <p>{val.time}</p>
            </div>
          ...
          <div className="flex gap-2 self-center">
            <Button
              ...
              onClick={() =>
                navigate(
                  `/admin-dashboard/edit-course/${type}/${val.titleSlug}`,
                )
              }
            >
              Edit
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    This action will remove the courses. Are you sure want to
                    delete?
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogCancel>Cancel</AlertDialogCancel>
                <AlertDialogAction onClick={() => handleDelete(val._id)}>
                  Delete
                </AlertDialogAction>
              </AlertDialogContent>
            </AlertDialog>
  ...
  return (
    <div className="w-full">
      <div className="border border-yellow-400 flex gap-4 p-2">
        ...
        <Button type="button" onClick={() => setType("listening")}>
          Listening
        </Button>
        <Button type="button" onClick={() => setType("speaking")}>
          Speaking
        </Button>
        <Button type="button" onClick={() => setType("reading")}>
          Reading
        </Button>
      </div>
